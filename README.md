...
<head>
    ...
    <title>Workplace & Weather Lab</title>
    <style>
        :root {
...
<script>
    const data = [
        { word: "forecast", def: "A prediction of future events (weather/business).", sents: ["What is the weather ___ for tomorrow?", "The sales ___ looks very positive."] },
        { word: "colleague", def: "A person that you work with.", sents: ["I am going to lunch with a ___.", "My ___ helped me finish the report."] },
        { word: "among", def: "In the middle of or surrounded by a group.", sents: ["He was standing ___ the crowd.", "There is a secret ___ us."] },
        { word: "crowded", def: "Full of people; very busy.", sents: ["The train was very ___ this morning.", "I don't like ___ shopping malls."] },
        { word: "expect", def: "To think that something will happen.", sents: ["I ___ to see him at the party.", "We ___ high results from this project."] },
        { word: "field", def: "An area of work or study.", sents: ["She is famous in the ___ of science.", "I want to change my career ___."] },
        { word: "minus", def: "Used to show temperatures below zero.", sents: ["It is ___ five degrees outside today.", "Water freezes at zero, not ___ ten."] },
        { word: "wear", def: "To have clothing or jewelry on your body.", sents: ["What are you going to ___ to the wedding?", "I always ___ a watch."] },
        { word: "take", def: "To move or carry something from one place to another.", sents: ["Don't forget to ___ your umbrella.", "Can you ___ these files to the office?"] }
    ];
    ...
    function init() {
        ...
        shuffle(pPool);
        document.getElementById('pron-content').innerHTML = pPool.map((s, i) => `<div class="gap-sentence">${s.t.replace(s.a, '_________')} <button class="mic-btn" onclick="testSpeech('${s.a}', 'p-fb-${i}')">🎤</button><div id="p-fb-${i}" style="font-size: 0.85em; color:#888; margin-top:5px;">Identify and speak the word!</div></div>`).join('');
    }
    ...
    function checkWriting() {
        document.querySelectorAll('.gap-input').forEach(input => {
            ...
            else if (dist <= 2) { fb.innerHTML = `<span class="status-yellow">⚠️ Typo!</span> <span class="correction-note">Correction: "${input.dataset.ans}"</span>`; totalScore += 0.5; }
